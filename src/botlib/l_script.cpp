...
struct line_t {
	...
	line_t(int vline) : vline(vline) {}
	line_t(int vline, std::string str) : vline(vline), str(str) {}
	bool operator() (const line_t& m) const {
		return m.vline == vline;
	}
};

int MV_MenuPatchFile(const char *in, size_t inhash, const char *patch, char **out);
std::vector<line_t> MV_MenuLineize(std::string const& str);
std::vector<line_t>::iterator MV_MenuIteratorByVLine(std::vector<line_t>& file, int vline);
std::istream& MV_MenuLineizeGetline(std::istream& is, std::string& t);
...
int MV_MenuPatchFile(const char *in, unsigned long inhash, const char *patch, char **out) {
	...
	std::vector<line_t> patchfile = MV_MenuLineize(std::string(patch));

	for (size_t i = 0; i < patchfile.size(); i++) {
		line_t cmdline = patchfile.at(i);

		// tokenize command line
		std::vector<std::string> cmd;
		std::string buf;
		std::stringstream ss(cmdline.str);
		while (ss >> buf) {
			cmd.push_back(buf);
		}
		...
		if (cmd.size() >= 1) {
			...
			} else if (cmd.at(0) == "LINES_DELETE") {
				int from, num;
				from = atoi(cmd.at(1).c_str());
				if (cmd.size() >= 3) {
					// LINES_DELETE FROM TO
					num = atoi(cmd.at(2).c_str()) + 1 - from;
				} else {
					// LINES_DELETE LINE
					num = 1;
				}

				std::vector<line_t>::iterator it = MV_MenuIteratorByVLine(menufile, from);
				menufile.erase(it, it + num);
			} else if (cmd.at(0) == "LINES_INSERT") {
				int into = atoi(cmd.at(1).c_str());
				std::vector<line_t>::iterator it = MV_MenuIteratorByVLine(menufile, into) + 1;

				std::vector<line_t> inslines;
				line_t currline = patchfile.at(++i);
				while (currline.str != "]") {
					currline.vline = 0;
					inslines.push_back(currline);
					currline = patchfile.at(++i);
				}

				menufile.insert(it, inslines.begin(), inslines.end());
			} else if (cmd.at(0) == "LINES_REPLACE") {
				int at = atoi(cmd.at(1).c_str());
				std::vector<line_t>::iterator it = MV_MenuIteratorByVLine(menufile, at);
...
std::vector<line_t> MV_MenuLineize(std::string const& str) {
	...
	while (!MV_MenuLineizeGetline(ss, linestr).eof()) {
		line_t line(vline, linestr);
		out.push_back(line);
		vline++;
	}
...
// handles dos, unix, mac line endings
std::istream& MV_MenuLineizeGetline(std::istream& is, std::string& t) {
	t.clear();
