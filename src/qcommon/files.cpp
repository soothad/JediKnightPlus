	...
	*listbuf = 0;
	...
	for (i =0; i < nFiles; i++) {
		...
		if (nTotal + nLen + 1 < bufsize) {
			...
			Com_DPrintf(S_COLOR_YELLOW "WARNING: FS_GetFileList: buffer size %d is too small for path:\"%s\" ext:\"%s\" (%d files dropped)\n", bufsize, path, extension, nFiles - i + 1);
			nFiles = i;
			break;
		}
	}
...
void FS_GetNextFile(char *path, int count)
{
	...
	strncpy(path, fileList[fileCurrent], (size_t) count);
	fileCurrent++;
}
