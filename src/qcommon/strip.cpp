...
int FindToken(char *token, bool whole)
{
	...
	for(token_value = 0; token_value != TK_END; token_value++)
	{
		...
		{
			if (Q_stricmpn(token, Tokens[token_value], (int)strlen(Tokens[token_value])) == 0)
			{
				i = (int)strlen(Tokens[token_value]);
				while(token[i] == ' ')
				{
					i++;
				}
...
bool cStringPackage::Load(char *FileName)
{
	...
	Size = (int)fread(buffer, 1, Size, FH);
	fclose(FH);
