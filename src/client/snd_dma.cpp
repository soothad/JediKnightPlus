...
void SND_setup()
{
	s_soundpoolmegs = Cvar_Get("s_soundpoolmegs", "25", CVAR_ARCHIVE | CVAR_GLOBAL);
...
void InitEAXManager()
{
	...
	if (s_bEAX)
	{
		s_hEAXManInst = LoadLibraryA("EAXMan.dll");
		if (s_hEAXManInst)
		{
...
void EAXMorph()
{
	...
	if (curPos >= 10)
	{
		...
		s_eaxSet(&DSPROPSETID_EAX_ListenerProperties, DSPROPERTY_EAXLISTENER_ALLPARAMETERS,
			0, &s_eaxLPSource, sizeof(EAXLISTENERPROPERTIES));
	}
	else
	{
		if (curPos > s_eaxMorphCount)
		{
			...
			s_eaxSet(&DSPROPSETID_EAX_ListenerProperties, DSPROPERTY_EAXLISTENER_ALLPARAMETERS,
				0, &s_eaxLPCur, sizeof(EAXLISTENERPROPERTIES));
